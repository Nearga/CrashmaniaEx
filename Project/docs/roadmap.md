...
### 4.7 Sticky Search & Category Chips
...
### 4.8 Skeleton Loading Placeholders
...
## Phase 5 — Balance, Currency & Store
...
## Phase 6 — Game Scene Shell & Crash Game Machine
...
## Phase 9 — Modal System & Shared UI Polish
*Goal: All modals use consistent enter/exit animations. Toast notifications work.*

### 9.1 ModalManager
- [ ] `ModalView.cs` — Canvas sort order 200; queue of pending modal prefab requests; shows one at a time
- [ ] `ModalMediator.cs` — listens to `ShowModal` / `HideModal` notifications
- [ ] Entry animation: DOTween scale `0.8→1.0` + CanvasGroup fade `0→1` over 0.25s
- [ ] Exit animation: reverse; then dequeue next modal if any
- [ ] Tap outside (background overlay button) triggers dismiss

### 9.2 Toast Notification
- [ ] `ToastView.cs` — anchored top-centre; slide down 0.3s, hold 2s, slide up 0.3s
- [ ] `ShowToast` notification with message string parameter

### 9.3 DOTween Animation Pass
Review all animations against spec targets:
- [ ] Scene transitions: fade 0.25s in + 0.25s out
- [ ] Card press: scale punch 0.15s
- [ ] Balance counter: 0.5s ease-out cubic
- [ ] Carousel snap: 0.3s
- [ ] Tab switch: color + scale 0.15s
